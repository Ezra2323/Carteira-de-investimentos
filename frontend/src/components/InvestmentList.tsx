    ...
    return (
        <div className="glass-panel overflow-hidden border-0">
            <div className="p-4 bg-white bg-opacity-10 border-bottom border-white border-opacity-10 d-flex justify-content-between align-items-center">
                ...
                <select
                    className="form-select form-select-sm w-auto bg-dark text-white border-secondary rounded-pill px-3"
                    onChange={(e) => setTypeFilter(e.target.value)}
                >
                    {/* O valor vazio "" faz o backend retornar todos os ativos */}
                    <option value="">Todos os Ativos</option>

                    {/* O 'value' deve ser EXATAMENTE igual ao nome no seu Enum AssetType no Java */}
                            ...
                            <td className="text-end px-4">
                                ...
                                {editingId === inv.id ? (
                                    <div className="d-flex gap-2 justify-content-end">
                                        <input
                                            type="number"
                                            className="form-control form-control-sm bg-dark text-white border-primary w-50"
                                            value={newPrice}
                                            onChange={(e) => setNewPrice(e.target.value)}
                                            placeholder="Novo R$"
                                            autoFocus
                                        />
                                        <button className="btn btn-sm btn-success" onClick={() => handleUpdatePrice(inv.id)}>
                                            ✓
                                        </button>
                                        <button className="btn btn-sm btn-outline-light" onClick={() => setEditingId(null)}>
                                            ✕
                                        </button>
                                    </div>
                                ) : (
                                    ...
                                    <button
                                        ...
                                        onClick={() => {
                                            setEditingId(inv.id);
                                            setNewPrice(inv.currentPrice?.toString() || "");
                                        }}
                                    >
                                        ⚙️
                                    </button>
