...
public class InvestmentService {
    ...
    public Map<String, Object> getSummary() {
        ...
        Map<AssetType, BigDecimal> totalByType = new HashMap<>();

        for (Investment inv : all) {
            BigDecimal qty = BigDecimal.valueOf(inv.getQuantity());

            // Custo de compra
            totalInvested = totalInvested.add(inv.getPurchasePrice().multiply(qty));

            // Valor de mercado (usa preço atual ou preço de compra se atual for null)
            BigDecimal marketPrice = (inv.getCurrentPrice() != null) ? inv.getCurrentPrice() : inv.getPurchasePrice();
            currentTotalValue = currentTotalValue.add(marketPrice.multiply(qty));

            // Soma por tipo
            totalByType.put(inv.getType(), totalByType.getOrDefault(inv.getType(), BigDecimal.ZERO).add(marketPrice.multiply(qty)));
        }

        Map<String, Object> result = new HashMap<>();
        result.put("totalInvested", totalInvested);
        result.put("currentTotalValue", currentTotalValue); // CAMPO NOVO
        result.put("totalProfitOrLoss", currentTotalValue.subtract(totalInvested)); // CAMPO NOVO
